...
#
# This script is part of the "Savage C2 Framework": 
...
class Main_prompt:
	...
	original_prompt = prompt = f"{UNDERLINE}Savage{END} > "
	hoax_prompt = None
	ready = True
	SPACE = '#>SPACE$<#'
	exec_active = False
...
class PrompHelp:
	...
	commands = {
		...
		'connect' : {
			'details' : f''' 			
Connect with another instance of Savage (sibling server). Once connected, you will be able to see and interact with foreign shell sessions owned by sibling servers and vice-versa. Multiple sibling servers can be connected at once. The limit of connections depends on the number of active threads a Savage instance can have (adjustable). In case you forgot the team server port number (default: 6501), use "sockets" to list Savage related services info. Read the Usage Guide or check my YouTube channel (@HaxorTechTones) for details.
...
connect <IP> <TEAM_SERVER_PORT>''',
		...
		'generate' : {
			'details' : f''' 		
Generate a reverse shell command. This function has been redesigned to use payload templates, which you can find in Savage/Core/payload_templates and edit or create your own.
...
''',
		...
		'repair' : {
			'details' : f''' 			
Use this command to manually correct a shell session's hostname/username value, in case Savage does not interpret the information correctly when the session is established.
...
Siblings are basically other instances of Savage that you are connected with.''',
			'least_args' : 0,
			'max_args' : 0
		},
		...
		'threads' : {
			'details' : f'''
Savage creates a lot of threads to be able to handle multiple shell sessions, connections with siblings and more. In file Savage/Core/settings.py there is a BoundedSemaphore that works as a thread limiter to prevent resource contention, set by default to 100 (you can of course change it). This command lists the active threads created by Savage, to give you an idea of what is happening in the background, what is the current value of the thread limiter etc.
...
Note that, if the thread limiter reaches 0, weird things will start happening as new threads (e.g. shell sessions) will be queued until: thread limiter > 0.''',
		...
		'sockets' : {
			'details' : f'''Prints Savage related socket services info.''',
			'least_args' : 0,
			'max_args' : 0
		},
...
upload <LOCAL_FILE_PATH> <REMOTE_FILE_PATH>''',
		...
		'cmdinspector' : {
			'details' : f'''
Savage has a function that inspects user issued shell commands for input that may cause a shell shell session to hang (e.g., unclosed single/double quotes or backticks, commands that may start a new interactive session within the current shell and more). 
...
conptyshell <IP or INTERFACE> <PORT> <SESSION ID or ALIAS>''',
		...
		'flee' : {
			'details' : f'''Quit without terminating active sessions. When you start Savage again, if any HoaxShell implant is still running on previously injected hosts, the session(s) will be re-established.''',
			'least_args' : 0,
			'max_args' : 0
		},
		...
		'purge' : {
			'details' : f'''Savage automatically stores information regarding generated implants and loads them in memory every time it starts. This way, HoaxShell generated implants become reusable and it is possible to re-establish older sessions, assuming the payload is still running on the victim(s). Use this command to delete all session related metadata. It does not affect any active sessions you may have.''',
			'least_args' : 0,
			'max_args' : 0
		},
		...
		'redirectors' : {
			'details' : f'''
Savage instances set traffic redirectors when using a shell session that belongs to a sibling. Use this command to list or remove redirectors, if required. 

Setting redirectors is handled automatically by Savage. Normally, users are not supposed to manually set redirectors. This feature exists purely for troubleshooting.
...
''',
	...
	def print_main_help_msg():
		...
		\r  sockets              Print Savage related running services' info.
